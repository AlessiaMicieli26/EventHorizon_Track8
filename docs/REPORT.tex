...
\title{\textbf{Domain Adaptation su MRI ADNI con VAE, AdaIN e CycleGAN}\\
...
\begin{abstract}
Questa relazione descrive il lavoro svolto per costruire una pipeline di domain adaptation su immagini di risonanza magnetica strutturale provenienti da ADNI. Il problema affrontato e' la variazione di dominio causata da scanner e produttori diversi: un classificatore addestrato su scansioni acquisite da alcuni produttori puo' degradare quando viene valutato su immagini acquisite da un produttore non visto. La pipeline prepara volumi T1, costruisce split patient-level usando due produttori come source domain e un terzo produttore come target domain, addestra un classificatore baseline source-only, integra traduzioni CycleGAN, genera augmentation tramite VAE scanner signatures e usa una variante AdaIN per il trasferimento di statistiche di stile scanner. Il codice e' stato organizzato secondo la struttura del repository: preprocessing in \texttt{src/datasets}, architetture in \texttt{src/models}, training in \texttt{src/training}, valutazione in \texttt{src/evaluation}, checkpoint in \texttt{experiments/checkpoints}, log in \texttt{experiments/logs} e figure in \texttt{figures}. Nella valutazione finale sincronizzata, addestrando su SIEMENS e GE MEDICAL SYSTEMS e testando su PHILIPS MEDICAL SYSTEMS, \texttt{source\_only} raggiunge accuracy 0.6571 e macro-F1 0.3966; \texttt{cyclegan\_augmented} raggiunge accuracy 0.4286 e macro-F1 0.4281; \texttt{vae\_gan\_augmented} raggiunge accuracy 0.6286 e macro-F1 0.3860; \texttt{adain\_gan\_augmented} raggiunge accuracy 0.7143 e macro-F1 0.6500. In questa configurazione AdaIN+GAN e' il modello migliore fra quelli confrontati, soprattutto per macro-F1.
\end{abstract}
...
La risposta empirica, nella run finale sincronizzata, e' che la pipeline e' eseguibile end-to-end e che il ramo AdaIN+GAN migliora il controllo source-only sul target PHILIPS. Il baseline raggiunge accuracy 0.6571 e macro-F1 0.3966, il modello CycleGAN-augmented ottiene 0.4286/0.4281, il modello VAE+GAN-augmented ottiene 0.6286/0.3860 e il modello AdaIN+GAN-augmented ottiene 0.7143/0.6500. Questo risultato e' utile perche' distingue due comportamenti: le augmentation VAE e CycleGAN non bastano a stabilizzare la classificazione, mentre il trasferimento AdaIN produce un miglior equilibrio fra classe disease e classe healthy.
...
\begin{itemize}[leftmargin=1.2cm]
    ...
    \item \texttt{data/ADNI1\_Complete 3Yr 3T.zip};
    \item \texttt{data/ADNI1\_Complete\_3Yr\_3T\_IDA\_Metadata.zip};
    \item \texttt{data/ADNI1\_Complete\_3Yr\_3T\_5\_29\_2026.csv}.
\end{itemize}
...
Il modello source-only e' il controllo sperimentale: viene addestrato solo su \texttt{source\_train.csv}, cioe' su SIEMENS e GE MEDICAL SYSTEMS, e viene valutato su \texttt{target\_test.csv}, cioe' su PHILIPS MEDICAL SYSTEMS. Il modello CycleGAN-augmented usa un training set che contiene sia source originali sia traduzioni source-to-target. Il modello VAE+GAN-augmented usa un training set da 243 righe dati: 81 slice source originali, 81 slice sintetiche generate tramite firme VAE e 81 slice tradotte tramite GAN. Anche il modello AdaIN+GAN-augmented usa 243 righe: 81 originali, 81 versioni AdaIN e 81 traduzioni CycleGAN. La valutazione e' stata eseguita solo sul target test set PHILIPS, composto da 35 scansioni non usate per il training.

Nella run sincronizzata, AdaIN+GAN e' il modello migliore: migliora il baseline di +0.0571 in accuracy e +0.2534 in macro-F1. Il miglioramento della macro-F1 e' particolarmente rilevante perche' il baseline source-only classifica correttamente la classe healthy ma non recupera campioni disease nella run finale.

\begin{table}[H]
\centering
\caption{Metriche per classe del modello AdaIN+GAN-augmented. Classe 0: disease, classe 1: healthy.}
\begin{tabular}{lrrrr}
\toprule
...
La Figura~\ref{fig:multi_scatter} e' il grafico generato dalla valutazione multi-modello e salvato direttamente in \texttt{figures/08.png}. Il grafico posiziona ogni modello nello spazio accuracy/macro-F1 sul target domain. La Figura~\ref{fig:scatter} e' invece la valutazione source-only vs CycleGAN-augmented mantenuta come artefatto precedente.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.74\textwidth]{figures/08.png}
    \caption{Confronto multi-modello sul target test set PHILIPS: source-only, CycleGAN-augmented, VAE+GAN-augmented e AdaIN+GAN-augmented.}
...
Il risultato principale della pipeline finale sincronizzata e' che AdaIN+GAN e' il modello migliore sul produttore non visto: accuracy 0.7143 e macro-F1 0.6500. Il baseline source-only raggiunge accuracy 0.6571 ma macro-F1 0.3966, perche' nella run finale predice correttamente la classe healthy ma non recupera campioni disease. CycleGAN-augmented ottiene accuracy 0.4286 e macro-F1 0.4281, mentre VAE+GAN-augmented ottiene accuracy 0.6286 e macro-F1 0.3860. AdaIN+GAN e' quindi l'unica variante che migliora chiaramente il controllo source-only sulla macro-F1.
...
Quinto, ne' il VAE ne' la CycleGAN sono vincolati da una loss diagnostica. Essi ottimizzano ricostruzione, regolarizzazione latente, plausibilita' visuale e consistenza ciclica, ma non necessariamente preservano caratteristiche legate alla malattia. In ambito medico, questa distinzione e' cruciale: una immagine plausibile dal punto di vista visivo puo' non essere una buona immagine per addestrare un classificatore diagnostico.
...
\begin{itemize}[leftmargin=1.2cm]
    ...
    \item \texttt{figures/08.png} (confronto quantitativo multi-modello);
    \item \texttt{figures/multi\_model\_accuracy\_f1.png} (confronto quantitativo multi-modello sincronizzato);
...
Il progetto ha prodotto una pipeline completa e riproducibile per studiare domain adaptation su MRI ADNI. Sono stati implementati preprocessing, split patient-level per dominio scanner, VAE per firme latenti di macchinari, AdaIN per statistiche di stile, generazione di immagini sintetiche, traduzione CycleGAN, training aumentato VAE+GAN e AdaIN+GAN, evaluation quantitativa e nuove figure qualitative della pipeline finale.

Il risultato finale e' che il modello AdaIN+GAN-augmented ottiene la migliore prestazione sul target test set PHILIPS MEDICAL SYSTEMS, con accuracy 0.7143 e macro-F1 0.6500. Il baseline source-only raggiunge 0.6571 e 0.3966, il modello CycleGAN-augmented 0.4286 e 0.4281, mentre il modello VAE+GAN-augmented 0.6286 e 0.3860. Il risultato indica che l'augmentation generativa non e' automaticamente utile: CycleGAN e VAE non migliorano il bilanciamento fra le classi, mentre AdaIN+GAN produce il miglior compromesso nella run sincronizzata.

Il lavoro svolto costituisce quindi una base solida per esperimenti successivi. La struttura del codice e' stata organizzata secondo le responsabilita' del repository, gli artefatti sono stati salvati nelle cartelle corrette e le figure nuove sono state prodotte in \texttt{figures/}, in particolare \texttt{figures/06.png}, \texttt{figures/07.png}, \texttt{figures/08.png}, \texttt{figures/09.png} e \texttt{figures/10.png}.
...
\begin{lstlisting}[language=bash]
python src/datasets/prepare_adni.py --csv data/ADNI1_Annual_2_Yr_3T_5_26_2026.csv --csv data/ADNI1_Complete_3Yr_3T_5_29_2026.csv --nifti-root data/raw/nifti --metadata-root data/raw/metadata --out-dir data/processed/00_prepared --field 3T --label-mode cn_vs_ad
\end{lstlisting}
...
Il ramo AdaIN segue la stessa logica del VAE, ma trasferisce statistiche di feature invece di spostare vettori latenti globali. Nella run sincronizzata il relativo \texttt{metrics.json} e' stato prodotto ed e' incluso nel confronto finale.
...
La valutazione a due modelli genera \texttt{experiments/outputs/evaluation/summary.md}. La valutazione multi-modello genera \texttt{experiments/outputs/evaluation\_multi/summary.md} e il grafico in \texttt{figures/08.png}; nella run sincronizzata e' presente anche \texttt{figures/multi\_model\_accuracy\_f1.png}. Tutte le nuove figure devono rimanere nella cartella \texttt{figures/}, cosi' la relazione puo' includerle con un path unico.
...
python src/evaluation/evaluate_multi_pipeline.py --model source_only=experiments/outputs/classifier_source_only/metrics.json --model cyclegan_augmented=experiments/outputs/classifier_cyclegan_augmented/metrics.json --model vae_gan_augmented=experiments/outputs/classifier_signature_gan_augmented/metrics.json --model adain_gan_augmented=experiments/outputs/classifier_adain_gan_augmented/metrics.json --out-dir experiments/outputs/evaluation_multi --figures-dir figures
\end{lstlisting}
