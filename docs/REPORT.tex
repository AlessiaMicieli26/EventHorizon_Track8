...
\begin{abstract}
Questa relazione descrive il lavoro svolto per costruire una pipeline di domain adaptation su immagini di risonanza magnetica strutturale provenienti da ADNI. Il problema affrontato e' la variazione di dominio causata da scanner e produttori diversi: un classificatore addestrato su scansioni acquisite da alcuni produttori puo' degradare quando viene valutato su immagini acquisite da un produttore non visto. La pipeline prepara volumi T1, costruisce split patient-level usando due produttori come source domain e un terzo produttore come target domain, addestra un classificatore baseline source-only, integra traduzioni CycleGAN, genera augmentation tramite VAE scanner signatures e predispone anche una variante AdaIN per il trasferimento di statistiche di stile scanner. Il codice e' stato organizzato secondo la struttura del repository: preprocessing in \texttt{src/datasets}, architetture in \texttt{src/models}, training in \texttt{src/training}, valutazione in \texttt{src/evaluation}, checkpoint in \texttt{experiments/checkpoints}, log in \texttt{experiments/logs} e figure in \texttt{figures}. Nella valutazione finale disponibile, addestrando su SIEMENS e GE MEDICAL SYSTEMS e testando su PHILIPS MEDICAL SYSTEMS, il baseline \texttt{source\_only} raggiunge accuracy 0.7429 e macro-F1 0.7395; \texttt{cyclegan\_augmented} raggiunge accuracy 0.5714 e macro-F1 0.5585; \texttt{signature\_gan\_augmented} raggiunge accuracy 0.5143 e macro-F1 0.4218. In questa configurazione le augmentation generative non migliorano il baseline, ma la pipeline rimane completa, riproducibile e utile per analizzare i limiti del setup sperimentale.
\end{abstract}
...
La risposta empirica, nella run finale disponibile, e' che la pipeline e' eseguibile end-to-end ma le augmentation generative non migliorano il controllo source-only. Il baseline raggiunge accuracy 0.7429 e macro-F1 0.7395 su PHILIPS, mentre il modello CycleGAN-augmented scende a 0.5714/0.5585 e il modello VAE+GAN-augmented a 0.5143/0.4218. Questo risultato e' comunque utile, perche' mostra una pipeline funzionante e mette in evidenza limiti tecnici e sperimentali importanti: numero ridotto di campioni, training generativo non supervisionato, uso di una sola slice centrale per volume, assenza di validazione qualitativa clinica e architetture volutamente leggere.
...
VAE+GAN-augmented & 0.5143 & 0.4218 & -0.2286 & -0.3177 \\
\bottomrule
\end{tabular}
\end{table}

Il modello source-only e' il controllo sperimentale: viene addestrato solo su \texttt{source\_train.csv}, cioe' su SIEMENS e GE MEDICAL SYSTEMS, e viene valutato su \texttt{target\_test.csv}, cioe' su PHILIPS MEDICAL SYSTEMS. Il modello CycleGAN-augmented usa invece un training set che contiene sia source originali sia traduzioni source-to-target. Il modello VAE+GAN-augmented usa un training set da 243 righe dati: 81 slice source originali, 81 slice sintetiche generate tramite firme VAE e 81 slice tradotte tramite GAN. La valutazione e' stata eseguita solo sul target test set PHILIPS, composto da 35 scansioni non usate per il training. La variante AdaIN+GAN e' predisposta nello script completo, ma non e' inclusa nella tabella perche' nella run valutata non era presente il relativo \texttt{metrics.json}.

Dopo l'analisi qualitativa e' stata corretta la generazione VAE signature da decoder diretto a generazione residuale. Le figure qualitative e il CSV aumentato sono stati rigenerati con questa versione corretta; le metriche quantitative in tabella restano quelle della run disponibile prima del riaddestramento del classificatore VAE+GAN con le nuove immagini residuali. Per aggiornare anche i numeri occorre rilanciare il training di \texttt{classifier\_signature\_gan\_augmented}.
...
\begin{figure}[H]
    ...
    \caption{Confronto multi-modello sul target test set PHILIPS: source-only, CycleGAN-augmented e VAE+GAN-augmented.}
...
Il risultato principale della pipeline finale e' che il classificatore source-only rimane il modello migliore sul produttore non visto: accuracy 0.7429 e macro-F1 0.7395. Le augmentation generative peggiorano entrambe il risultato: CycleGAN-augmented ottiene accuracy 0.5714 e macro-F1 0.5585, mentre VAE+GAN-augmented ottiene accuracy 0.5143 e macro-F1 0.4218. Per il baseline source-only la classe disease ottiene F1-score 0.7097 e la classe healthy ottiene F1-score 0.7692. Questo conferma che, nella run disponibile, il modello piu' semplice e' anche quello che preserva meglio l'equilibrio fra le classi sul target test set.
...
\begin{itemize}[leftmargin=1.2cm]
    ...
    \item \texttt{adain\_signature.pt} (previsto dallo script completo, se viene eseguita la run AdaIN);
    ...
    \item \texttt{classifier\_adain\_gan\_augmented\_best\_model.pt} (previsto dallo script completo, se viene eseguita la run AdaIN).
\end{itemize}
...
\begin{itemize}[leftmargin=1.2cm]
    ...
    \item \texttt{experiments/outputs/adain\_signature/manifest.json} (previsto dallo script completo, se viene eseguita la run AdaIN);
...
\subsection{Ramo AdaIN predisposto}

Il ramo AdaIN segue la stessa logica del VAE, ma trasferisce statistiche di feature invece di spostare vettori latenti globali. Il classificatore AdaIN+GAN e' confrontabile solo quando il relativo \texttt{metrics.json} viene prodotto.
...
python src/evaluation/evaluate_multi_pipeline.py --model source_only=experiments/outputs/classifier_source_only/metrics.json --model cyclegan_augmented=experiments/outputs/classifier_cyclegan_augmented/metrics.json --model signature_gan_augmented=experiments/outputs/classifier_signature_gan_augmented/metrics.json --out-dir experiments/outputs/evaluation_multi --figures-dir figures
\end{lstlisting}
