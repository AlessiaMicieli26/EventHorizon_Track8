...
\author{Gruppo: EventHorizon \qquad Track: 8}
\date{2026}
...
\begin{itemize}[leftmargin=1.2cm]
    ...
    \item \texttt{docs/figures/}: figure usate nella relazione;
...
La componente VAE e' definita in \path{src/models/vae.py} e addestrata con \path{src/training/train_vae_signature.py}. L'obiettivo non e' classificare direttamente la malattia, ma proiettare le slice MRI in uno spazio latente in cui sia possibile stimare una firma media per ciascun produttore source.
...
La variante AdaIN e' definita in \path{src/models/adain.py}. Gli script di training e generazione sono \path{train_adain_signature.py} e \path{generate_adain_augmented.py}. L'idea e' evitare di chiedere al modello di generare tutta l'immagine partendo da un vettore latente globale, come nel VAE, e trasferire invece lo stile scanner tramite statistiche di feature.
...
La pipeline AdaIN produce un CSV dedicato, \path{data/processed/05_adain_signature_augmented/source_train_adain_signature_augmented.csv}, che puo' essere fuso con le traduzioni GAN nello stesso modo del dataset VAE. Il classificatore risultante viene salvato in \path{experiments/outputs/classifier_adain_gan_augmented}, cosi' da confrontarlo direttamente con \texttt{classifier\_signature\_gan\_augmented}.
...
La valutazione dei classificatori finali viene salvata nelle rispettive directory sotto \texttt{experiments/outputs/}. Lo script \texttt{src/evaluation/evaluate\_multi\_pipeline.py} confronta baseline, CycleGAN, VAE+GAN, AdaIN+GAN e oracle target-supervised e produce:
...
\begin{itemize}[leftmargin=1.2cm]
    ...
    \item figure in \texttt{docs/figures/}.
\end{itemize}
...
Il modello source-only e' il controllo sperimentale: viene addestrato solo su \path{source_train.csv}, cioe' su SIEMENS e GE MEDICAL SYSTEMS, e viene valutato su \path{target_test.csv}, cioe' su PHILIPS MEDICAL SYSTEMS. Il modello CycleGAN-augmented usa un training set che contiene sia source originali sia traduzioni source-to-target. Il modello VAE+GAN-augmented usa un training set da 243 righe dati: 81 slice source originali, 81 slice sintetiche generate tramite firme VAE e 81 slice tradotte tramite GAN. Il modello AdaIN+GAN-augmented usa lo stesso schema, sostituendo le immagini sintetiche VAE con augmentation AdaIN. Il modello oracle target-supervised e' invece un controllo separato: usa le label del target adaptation set PHILIPS, diviso in \path{target_oracle_train.csv} e \path{target_oracle_val.csv}, e viene poi valutato sullo stesso \path{target_test.csv}. La valutazione finale e' stata eseguita solo sul target test set PHILIPS, composto da 35 scansioni non usate per il training.
...
\begin{table}[H]
\centering
\caption{Metriche per classe del modello oracle target-supervised. Classe 0: disease, classe 1: healthy.}
\begin{tabular}{lrrrr}
\toprule
Classe & Precision & Recall & F1-score & Support \\
\midrule
...
\texttt{target\_oracle\_train.csv} & 27 \\
\texttt{target\_oracle\_val.csv} & 9 \\
\bottomrule
\end{tabular}
\end{table}
...
La Figura~\ref{fig:multi_scatter} e' il grafico generato dalla valutazione multi-modello e salvato direttamente in \texttt{docs/figures/multi\_model\_accuracy\_f1.png}. Il grafico posiziona ogni modello nello spazio accuracy/macro-F1 sul target domain, includendo anche il controllo oracle target-supervised. La Figura~\ref{fig:scatter} e' invece la valutazione source-only vs CycleGAN-augmented mantenuta come artefatto precedente.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.74\textwidth]{multi_model_accuracy_f1.png}
    \caption{Confronto multi-modello sul target test set PHILIPS: source-only, CycleGAN-augmented, VAE+GAN-augmented, AdaIN+GAN-augmented e oracle target-supervised.}
    \label{fig:multi_scatter}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.72\textwidth]{01.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.88\textwidth]{03.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.92\textwidth]{06.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.92\textwidth]{07.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.78\textwidth]{04.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.92\textwidth]{05.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.92\textwidth]{09.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.86\textwidth]{10.png}
...
Quarto, la validazione dei classificatori di domain adaptation viene fatta su \texttt{source\_val}, quindi su Siemens e GE. Questo non fornisce una stima diretta delle prestazioni target su Philips. Per il controllo oracle e' stata invece usata una validazione target dedicata, \texttt{target\_oracle\_val.csv}; anche in questo caso, pero', la validation contiene solo 9 scansioni, quindi la selezione del checkpoint e' molto rumorosa.
...
\small
    ...
    \item \texttt{classifier\_oracle\_best\_model.pt}.
\end{itemize}
...
\scriptsize
    ...
    \item \path{data/processed/03_vae_signature_augmented/source_train_vae_signature_augmented.csv};
    \item \path{data/processed/05_adain_signature_augmented/source_train_adain_signature_augmented.csv};
    \item \path{data/processed/04_signature_gan_augmented/source_train_signature_gan_augmented.csv};
    \item \path{data/processed/06_adain_gan_augmented/source_train_adain_gan_augmented.csv};
    \item \path{experiments/outputs/classifier_source_only/metrics.json};
    \item \path{experiments/outputs/classifier_cyclegan_augmented/metrics.json};
    \item \path{experiments/outputs/classifier_signature_gan_augmented/metrics.json};
    \item \path{experiments/outputs/evaluation_multi/summary.md};
    \item \texttt{docs/figures/08.png}.
\end{itemize}

I log sono mantenuti in \texttt{experiments/logs}; le figure usate dalla relazione sono in \texttt{docs/figures}. Per la pipeline finale e' stata generata anche una nuova figura qualitativa:

\begin{itemize}[leftmargin=1.2cm]
    \item \texttt{docs/figures/01.png};
    \item \texttt{docs/figures/02.png};
    \item \texttt{docs/figures/03.png};
    \item \texttt{docs/figures/04.png} (pipeline CycleGAN precedente);
    \item \texttt{docs/figures/05.png} (pipeline VAE+GAN precedente);
    \item \texttt{docs/figures/06.png} (preview qualitativa aggiornata);
    \item \texttt{docs/figures/07.png} (mappe di differenza aggiornate);
    \item \texttt{docs/figures/08.png} (confronto quantitativo multi-modello);
    \item \texttt{docs/figures/09.png} (preview qualitativa estesa);
    \item \texttt{docs/figures/10.png} (mappe di cambiamento assoluto estese).
\end{itemize}
...
Il lavoro svolto costituisce quindi una base solida per esperimenti successivi. La struttura del codice e' stata organizzata secondo le responsabilita' del repository, gli artefatti sono stati salvati nelle cartelle corrette e le figure nuove sono state copiate in \texttt{docs/figures/}, in particolare \texttt{docs/figures/06.png}, \texttt{docs/figures/07.png}, \texttt{docs/figures/08.png}, \texttt{docs/figures/09.png} e \texttt{docs/figures/10.png}.
...
Input principali: CSV ADNI, cartella NIfTI e cartella XML. Output principali: \path{data/processed/00_prepared/index_prepared.csv} e volumi \texttt{.npy} normalizzati.
...
Output: checkpoint in \path{experiments/checkpoints/classifier_source_only_best_model.pt} e metriche in \path{experiments/outputs/classifier_source_only/metrics.json}.
...
Il primo comando produce i checkpoint dei generatori e i log della loss. Il secondo applica il generatore source-to-target a ogni esempio source e salva \path{data/processed/02_translated_source/translated_source_train.csv}.
...
Output: \path{experiments/outputs/classifier_cyclegan_augmented/metrics.json}.
...
Output: \path{experiments/outputs/classifier_signature_gan_augmented/metrics.json}. Questo e' il modello indicato nelle tabelle come VAE+GAN-augmented.
...
\subsection{Valutazione e figure in \texttt{docs/figures/}}

La valutazione a due modelli genera \path{experiments/outputs/evaluation/summary.md}. La valutazione multi-modello con oracle genera \path{experiments/outputs/evaluation_multi_with_oracle/summary.md} e il grafico \path{docs/figures/multi_model_accuracy_f1.png}. Tutte le figure usate dal report devono rimanere nella cartella \path{docs/figures/}, cosi' la relazione puo' includerle con un path unico.

\begin{lstlisting}[language=bash]
python src/evaluation/evaluate_pipeline.py --baseline experiments/outputs/classifier_source_only/metrics.json --augmented experiments/outputs/classifier_cyclegan_augmented/metrics.json --out-dir experiments/outputs/evaluation --figures-dir docs/figures

python src/evaluation/evaluate_multi_pipeline.py --model source_only=experiments/outputs/classifier_source_only/metrics.json --model cyclegan_augmented=experiments/outputs/classifier_cyclegan_augmented/metrics.json --model vae_gan_augmented=experiments/outputs/classifier_signature_gan_augmented/metrics.json --model adain_gan_augmented=experiments/outputs/classifier_adain_gan_augmented/metrics.json --model oracle=experiments/outputs/classifier_oracle/metrics.json --out-dir experiments/outputs/evaluation_multi_with_oracle --figures-dir docs/figures
\end{lstlisting}
